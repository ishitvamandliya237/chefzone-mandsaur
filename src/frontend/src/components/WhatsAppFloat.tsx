import { useState } from "react";

export default function WhatsAppFloat() {
  const [waHovered, setWaHovered] = useState(false);
  const [igHovered, setIgHovered] = useState(false);

  return (
    <div
      className="fixed bottom-6 right-6 z-50 flex flex-col items-center gap-3"
      data-ocid="whatsapp.float_button"
    >
      {/* Instagram floating button */}
      <div className="relative">
        {/* Tooltip */}
        <div
          className="absolute bottom-full right-0 mb-2 px-3 py-1.5 rounded-lg text-xs font-semibold font-display whitespace-nowrap"
          style={{
            background: "rgba(5,5,16,0.95)",
            border: "1px solid rgba(0,245,255,0.2)",
            color: "#fff",
            boxShadow: "0 4px 16px rgba(0,0,0,0.4)",
            opacity: igHovered ? 1 : 0,
            transform: igHovered ? "translateY(0)" : "translateY(4px)",
            transition: "opacity 0.2s ease, transform 0.2s ease",
            pointerEvents: "none",
          }}
        >
          Follow on Instagram
        </div>
        <a
          href="https://www.instagram.com/chefzone_mandsaur"
          target="_blank"
          rel="noopener noreferrer"
          aria-label="Follow on Instagram"
          data-ocid="instagram.float_button"
          className="relative flex items-center justify-center w-12 h-12 rounded-full"
          style={{
            background: "linear-gradient(135deg, #833ab4, #fd1d1d, #fcb045)",
            boxShadow: igHovered
              ? "0 0 0 4px rgba(0,245,255,0.25), 0 0 24px rgba(225,48,108,0.6)"
              : "0 4px 20px rgba(225,48,108,0.4)",
            transform: igHovered ? "scale(1.1)" : "scale(1)",
            transition: "transform 0.2s ease, box-shadow 0.2s ease",
          }}
          onMouseEnter={() => setIgHovered(true)}
          onMouseLeave={() => setIgHovered(false)}
        >
          <svg
            viewBox="0 0 24 24"
            fill="white"
            className="w-5 h-5"
            aria-hidden="true"
          >
            <title>Instagram</title>
            <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
          </svg>
          <span className="sr-only">Follow on Instagram</span>
        </a>
      </div>

      {/* WhatsApp floating button */}
      <div className="relative">
        {/* Tooltip */}
        <div
          className="absolute bottom-full right-0 mb-2 px-3 py-1.5 rounded-lg text-xs font-semibold font-display whitespace-nowrap"
          style={{
            background: "rgba(5,5,16,0.95)",
            border: "1px solid rgba(212,175,55,0.2)",
            color: "#fff",
            boxShadow: "0 4px 16px rgba(0,0,0,0.4)",
            opacity: waHovered ? 1 : 0,
            transform: waHovered ? "translateY(0)" : "translateY(4px)",
            transition: "opacity 0.2s ease, transform 0.2s ease",
            pointerEvents: "none",
          }}
        >
          Order on WhatsApp
        </div>

        {/* Pulsing ring */}
        <span
          className="absolute inset-0 rounded-full"
          style={{
            animation: "waPulse 2s ease-out infinite",
            background: "rgba(37,211,102,0.35)",
            pointerEvents: "none",
          }}
        />

        <style>{`
          @keyframes waPulse {
            0% { transform: scale(1); opacity: 0.7; }
            70% { transform: scale(1.7); opacity: 0; }
            100% { transform: scale(1.7); opacity: 0; }
          }
        `}</style>

        <a
          href="https://wa.me/918889431668"
          target="_blank"
          rel="noopener noreferrer"
          aria-label="Order on WhatsApp"
          className="relative flex items-center justify-center w-14 h-14 rounded-full"
          style={{
            background: "linear-gradient(135deg, #25d366, #1ebe5d)",
            boxShadow: waHovered
              ? "0 0 0 4px rgba(37,211,102,0.25), 0 0 24px rgba(37,211,102,0.5)"
              : "0 4px 20px rgba(37,211,102,0.35)",
            transform: waHovered ? "scale(1.1)" : "scale(1)",
            transition: "transform 0.2s ease, box-shadow 0.2s ease",
          }}
          onMouseEnter={() => setWaHovered(true)}
          onMouseLeave={() => setWaHovered(false)}
        >
          <svg
            viewBox="0 0 24 24"
            fill="white"
            className="w-7 h-7"
            aria-hidden="true"
          >
            <title>WhatsApp</title>
            <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z" />
            <path d="M12 0C5.373 0 0 5.373 0 12c0 2.123.554 4.116 1.528 5.844L.057 23.888l6.21-1.63A11.94 11.94 0 0012 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 21.818a9.818 9.818 0 01-4.999-1.365l-.358-.213-3.688.968.985-3.595-.234-.371A9.818 9.818 0 012.182 12C2.182 6.58 6.58 2.182 12 2.182S21.818 6.58 21.818 12 17.42 21.818 12 21.818z" />
          </svg>
          <span className="sr-only">Order on WhatsApp</span>
        </a>
      </div>
    </div>
  );
}
